...
class BaseGenerator(nn.Module):
    ...
    def forward(self, inputs):
        #print(inputs.shape)
        #print(np.shape(inputs.contiguous().view(-1, inputs.size(-1))))
    ...
    def backward(self, outputs, targets, weights, normalizer, criterion, regression=False):
        ...
        return grad_output, loss.item()  #loss changed to loss.item()
    ...
    def predict(self, outputs, targets, weights, criterion):
        ...
        return preds, loss.item()  #loss changed to loss.item()
