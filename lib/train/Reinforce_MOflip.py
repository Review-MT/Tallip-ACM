...
class Reinforce_MOflip(object):
    ...
    def train_epoch(self, epoch, no_update):
        ...
        for i in range(len(self.train_data)):
            ...
            #=================================================
            # Sample translations
            #attention_mask = sources[0].data.eq(lb.Constants.PAD).t()   #this logic is shifted to n_sample fun
            #self.actor.decoder.attn.applyMask(attention_mask)
            #=================================================
            
            samples, outputs, _completed_samples, gold, sample_scores = self.actor.n_sample(batch, self.max_length)
           
            #Calculate rewards  
            reinfore_rewards , select_sample_id = self.max_reward_sample(_completed_samples, targets , gold, sample_scores, self.dicts, batch_size)
            ...
            print(samples) 
            decoded = samples.transpose(0,1)
            
            print("Maximum rewarding Target Sample")
            sent = [self.dicts["tgt"].convertToLabels(s.tolist(), stop=3)[:-1] for s in decoded]
            print(sent) 
            ...
            samples = Variable(torch.LongTensor(samples).t().contiguous()) #as sample is not returned from reward func
...
#=======================================================================
    ...
    def max_reward_sample(self, raw_samples, targets, gold, sample_scores, dicts, batch_size,  \
           sample_size=5):
        '''
        ...
        if self.opt.reward_model == 'sent_reward':
           ...
           sent_samples = self.convert_index2Label_topfive(raw_samples, sample_scores, dicts, batch_size)
    ...
    def convert_index2Label_topfive(self, _completed_samples, sample_scores, dicts, batch_size, sample_size=5):
