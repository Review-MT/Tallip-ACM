...
#=====================================        
        ...
        train_data = self.train_data[0:int(c0*len(self.train_data))]
        
        #Temp fix, add as agrument later
        gradient_accumulation_steps = 1
        num_train_epochs = 20
        t_total = len(self.train_data) // args.gradient_accumulation_steps * args.num_train_epochs
        step = 0
        for i in range(len(train_data)):
            

            current_data_iter = len(train_data)
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
            ...
            percentage_data_by_epoch = 1.0
            curriculum_iterations = (t_total * percentage_data_by_epoch) * percentage_curriculum_iter
            
            #new_data_fraction = min(1,PACING_FUNCTIONS[args.pacing_function](global_step, curriculum_iterations, c0))
            
            new_data_fraction = min(1, self.root_2(global_step, curriculum_iterations, c0))
            ...
            train_data = self.train_data[0:int(c0*int(new_data_fraction*len(self.train_data))]

            if step == int(percentage_data_by_epoch * (t_total/num_train_epochs)):
