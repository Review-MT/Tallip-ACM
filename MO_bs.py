...
parser = argparse.ArgumentParser(description="Reinforce_baseline_flip.py")
...
def main():
    ...
    #For all Flipkard data Fine Tuning
    supervised_data = lb.Dataset(dataset["train_iitb"], opt.batch_size, opt.cuda, name = "train_iitb", eval=False)
    reinfore_data = lb.Dataset(dataset["train_pg"], opt.batch_size, opt.cuda, name ="train_pg", gold = True, eval=False)
    valid_data = lb.Dataset(dataset["valid_post"], opt.batch_size, opt.cuda, name ="valid_post", gold = True, eval=False)
    ...
    print(" * number of valid  sentences. %d" %
          len(dataset["valid_post"]["src"]))
    ...
    if opt.eval:
        ...
        reinforce_trainer = lb.Reinforce_MOflip(model, reinfore_data, test_data,
        ...
        if use_reinforce:
            ...
            # Reinforce training
            reinforce_trainer = lb.Reinforce_MOflip(model, reinfore_data, valid_data,
                    metrics, dicts, optim, opt)
