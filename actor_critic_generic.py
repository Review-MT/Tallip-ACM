...
parser = argparse.ArgumentParser(description="actor_critic_generic.py")
...
def create_critic(checkpoint, dicts, opt):
    if opt.load_from is not None and "critic" in checkpoint:
        print("critic already in check point")
        critic = checkpoint["critic"]
        critic_optim = checkpoint["critic_optim"]
    else:
        
        critic, critic_optim = create_model(lb.NMTModel, dicts, 1)
...
def main():
   ...
   # print(bandid_data)
    ...
    print(" * number of val training sentences. %d" %
          len(dataset["valid_pre"]["src"]))
    ...
    if opt.eval:
        ...
        reinforce_trainer = lb.ActorCritic(model, critic, reinforce_data, test_data,
        ...
        if use_critic:
           ...
           # Create critic here to not affect random seed.
            ...
            if opt.critic_pretrain_epochs > 0:
                reinforce_trainer = lb.ActorCritic(model, critic, supervised_data, valid_data,
                    metrics, dicts, optim, critic_optim, opt)
                reinforce_trainer.train(opt.start_reinforce,
                    opt.start_reinforce + opt.critic_pretrain_epochs - 1, True, start_time)     
            # Reinforce training
            reinforce_trainer = lb.ActorCritic(model,critic, reinfore_data, valid_data,
                    metrics, dicts, optim,critic_optim, opt)
